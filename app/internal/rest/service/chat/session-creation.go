...
func (s *service) SessionCreation(user models_rest.UserSignIn, accessTokenExp time.Duration, refreshTokenExp time.Duration, secretKey string) (models_rest.Tokens, models_rest.UserDB, error, string, string, int, string) {
	// validation
	...
	userDB, err, errKey, errMsg, code, fileInfo := s.repo.SessionCreation(user)
	if err != nil {
		return models_rest.Tokens{}, models_rest.UserDB{}, err, errKey, errMsg, code, fileInfo
	}
