...
func (s *Service) SignIn(user models_rest.UserSignIn, accessTokenExp time.Duration, refreshTokenExp time.Duration, secretKey string) (models_rest.Tokens, models_rest.UserSignIn, error, string, string, int, string) {
	// validation
	err, errKey, errMsg, code, fileInfo := validation_rest.ValidationSignIn(user.Email, user.Password)
	if err != nil {
		return models_rest.Tokens{}, models_rest.UserSignIn{}, err, errKey, errMsg, code, fileInfo
	}
